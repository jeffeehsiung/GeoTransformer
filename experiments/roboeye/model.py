...
from geoTransformer.GeoTransformer.experiments.roboeye.backbone import KPConvFPN
from geoTransformer.GeoTransformer.experiments.roboeye.utils import ensure_tensor
...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        feats = data_dict[
            "features"
        ].detach()  # TODO: train from scratch to use normals instead of ones features
        # Only use ground-truth transform if not in inference mode
        transform = data_dict.get("transform", None)
        if transform is not None:
            transform = ensure_tensor(transform, device=next(self.parameters()).device)
        ...
        if transform is not None:
            gt_node_corr_indices, gt_node_corr_overlaps = get_node_correspondences(
                ref_nodes=ref_points_c,
                src_nodes=src_points_c,
                ref_knn_points=ref_node_knn_points,
                src_knn_points=src_node_knn_points,
                transform=transform,
                pos_radius=self.matching_radius,
...
def create_model(config: edict) -> torch.nn.Module:
